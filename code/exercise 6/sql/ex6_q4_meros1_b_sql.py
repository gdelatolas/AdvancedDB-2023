...
spark.sql(  """
            SELECT p.Y, p.X, f.LAT, f.LON, p.PREC, p.DIVISION
            FROM police_table p
...
grouped_df = spark.sql( """
                        SELECT DIVISION, AVG(distance) AS mean_distance, COUNT(PREC) AS count_per_police_department
                        FROM result_table
                        GROUP BY DIVISION
                        ORDER BY count_per_police_department DESC
                        """)
