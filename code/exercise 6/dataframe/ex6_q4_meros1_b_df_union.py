...
filtered_df = df.filter(col("Weapon Used Cd").isNotNull())
...
# Group by "PREC" and compute the mean distance and count
grouped_df = result_df.groupBy("DIVISION").agg(
    mean("distance").alias("mean_distance"),
    count("DIVISION").alias("COUNT")
)

# Order the result by "PREC"
grouped_df = grouped_df.orderBy("COUNT", ascending=False)
