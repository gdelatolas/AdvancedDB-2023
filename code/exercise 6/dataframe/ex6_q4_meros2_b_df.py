...
filtered_df = df.na.drop(subset=["Weapon Used Cd"])
...
# Group by αστυνομικό τμήμα και υπολογισμός στατιστικών
grouped_df = closest_police_station_df.groupBy("DIVISION").agg(
    mean("distance").alias("mean_distance"),
    count("DIVISION").alias("incident_count"),
)

# Ταξινόμηση κατά αύξουσα σειρά του αριθμού περιστατικών
grouped_df = grouped_df.orderBy("incident_count", ascending=False)
