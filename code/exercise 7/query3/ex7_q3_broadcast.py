...
from pyspark.sql.functions import col, regexp_replace,to_timestamp, year, month, row_number, when, count, desc, udf, expr, broadcast
...
joined_df = reverse_df.join(broadcast(new_df.select("Zip Code")), reverse_df["ZIPcode"] == new_df["Zip Code"], "inner")
...
filtered_df = df.join(broadcast(unique_lat_lon_pairs), ["LAT", "LON"], "inner")
